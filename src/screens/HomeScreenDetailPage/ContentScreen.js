...
const ContentScreen = ({ route }) => {
  ...
  const processHtmlContent = htmlContent => {
    const imgRegex = /<img [^>]*src="\/(images\/post-images\/.*?)\"[^>]*>/g;
    const updatedHtml = htmlContent.replace(imgRegex, (match, p1) => {
      const newSrc = `https://e-psikiyatri.com/${p1}`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        fetchedContent.post = processHtmlContent(fetchedContent.post);
